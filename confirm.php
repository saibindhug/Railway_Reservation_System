...
<body>
	...
	<nav class="navbar sticky-top navbar-expand-sm navbar-dark bg-dark">
			...
			if(isset($_POST["trains"])){
				...
				$q="SELECT train_info.Train_No,Train_Name,$class,`Departure_Time` FROM train_info,train_schedule WHERE Source_Station_Name='$from' AND Destination_Station_Name='$to' AND train_schedule.Train_No=train_info.Train_No";
				$r=mysqli_query($db,$q);
				$q1="SELECT `Arrival_Time` FROM train_info,train_schedule WHERE Source_Station_Name='$from' AND Destination_Station_Name='$to' AND days<>'$y' AND train_schedule.Train_No=train_info.Train_No";
				...
				$y=$row['days'];
				$q="SELECT train_info.Train_No,Train_Name,$class2,`Departure_Time` FROM train_info,train_schedule WHERE Source_Station_Name='$from2' AND Destination_Station_Name='$to2' AND days<>'$y' AND train_schedule.Train_No=train_info.Train_No";
				$r=mysqli_query($db,$q);
				$q1="SELECT `Arrival_Time` FROM train_info,train_schedule WHERE Source_Station_Name='$from2' AND Destination_Station_Name='$to2' AND days<>'$y' AND train_schedule.Train_No=train_info.Train_No";
				$r1=mysqli_query($db,$q1);
				if(!$r||mysqli_num_rows($r)==0){
						echo "<h3>No Trains Found.</h3>";
					}
